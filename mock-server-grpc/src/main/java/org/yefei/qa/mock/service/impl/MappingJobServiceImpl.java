...
public class MappingJobServiceImpl implements IMappingJobService {
    ...
    @Override
    public void addJobs(TblGrpcRequestMapping tblGrpcRequestMapping, HashMap userDefined, HashMap params) {
        List<TblMappingJob> tblMappingJobs = requestJobCache.queryTblMappingJobsByRequestID(tblGrpcRequestMapping.getRequestID());
        ...
        if (tblMappingJobs != null && tblMappingJobs.size() > 0 ){
            ...
            taskSourceInfo.setGrpcRequestMapping(tblGrpcRequestMapping);
            taskSourceInfo.setSourceServer(SourceServerEnum.GRPC);

            tblMappingJobs.stream().forEach(tblMappingJob->{
                this.addJob(tblMappingJob, taskSourceInfo, userDefined, params);
            });
        }
    }

    private void addJob(TblMappingJob tblMappingJob, TaskSourceInfo taskSourceInfo, HashMap userDefined, HashMap params){
        try {
            List<TblMappingTask> tblMappingTasks = requestTaskCache.queryGrpcMappingRulesDetailsByRequestID(tblMappingJob.getJobID());
            ...
            callbackJobBean.setSourceServer(SourceServerEnum.GRPC.getSrc());
                ...
                Arrays.stream(TaskTypeEnum.values()).forEach(taskTypeEnumItem -> {
                    if (taskTypeEnumItem.getTaskType() == taskType) {
                        try {
                            ...
                            CallbackTaskBean callbackTaskBean = taskGenerator.generate(taskSourceInfo, requestTaskItem, params,userDefined, params);
