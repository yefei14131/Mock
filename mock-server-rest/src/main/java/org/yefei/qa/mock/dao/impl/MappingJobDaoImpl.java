...
public class MappingJobDaoImpl implements IMappingJobDao {
    ...
    @Override
    public List<TblMappingJob> queryRestMappingJobs(int requestID) {
        TblMappingJobExample example = new TblMappingJobExample();
        TblMappingJobExample.Criteria criteria = example.createCriteria();
        criteria.andRequestIDEqualTo(requestID).andProtocolEqualTo(ProtocolEnum.HTTP.getProtocol()).andIsActiveEqualTo(true);
