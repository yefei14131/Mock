...
public class ResponseWriterImpl implements IResponseWriter{
        ...
        if (recordedRequestMapping != null){
            ...
            for ( ResponseTypeEnum e :  ResponseTypeEnum.values()){
                if(e.getType().equals(responseType)){
                    ...
                    writeContent(response, recordedRequestMapping, userDefined, params, globalData);
                }
            }
        }
    ...
    private void writeContent(HttpServletResponse response, TblRestRequestMapping recordedRequestMapping, HashMap userDefined, HashMap params, HashMap globalData) throws IOException {
        ...
        String outputHeader = VariableManager.replaceContent(recordedRequestMapping.getResponseHeader(), userDefined, params, globalData);
        systemDebugger.addSystemLog("最终输出响应结果 header:", outputHeader);
        writeHeader(response, outputHeader);

        String outputContent = VariableManager.replaceContent(recordedRequestMapping.getResponseBody(), userDefined, params, globalData);
