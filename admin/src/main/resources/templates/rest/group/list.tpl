...
<div class="layui-body">
    ...
    <div class="content-head">
        ...
        <div class="head-container right">
            <a href="/mock_server/rest/group/info.html">
                <svg class="icon">
        ...
        -->
               ...
               <!--
                       <a class="layui-btn" href="/mock_server/rest/group/info.html">
                           <i class="layui-icon layui-icon-add-circle " title="新增"></i>
                       </a>

                       <button class="layui-btn layui-bg-orange group-import" id="group-import">
                           <i class="layui-icon " title="导入">导入</i>
                       </button>

                       <button class="layui-btn layui-bg-gray group-export" >
                           <i class="layui-icon" title="导出">导出</i>
                       </button>
               -->
        </div>
        <div class="clear"></div>
...
<script>
    ...
    layui.use(['table','upload', 'layer'], function(){
        ...
        var uploadInst = upload.render({
            ...
            ,url: '/mock_server/rest/group/import.ajax' //上传接口
            ,accept: 'file'
            // ,exts: 'data'
            ,done: function(res){
                //上传完毕回调
    ...
    function downloadGroupData(groupIDs) {
        var url = "/mock_server/rest/group/export.htm?groupIDs="+groupIDs
        window.open(url)
    }
    ...
    <script type="text/html" id="oprateBar">
        <a class="layui-btn layui-btn-xs" lay-event="detail" href="/mock_server/rest/mapping/list.html?groupID={{d.groupID}}&groupCode={{d.groupCode}}">
            <i class="layui-icon layui-icon-list layui-bg-green" title="接口列表"></i>
        </a>
        <a class="layui-btn layui-btn-xs layui-bg-green" lay-event="edit" href="/mock_server/rest/group/info.html?groupID={{d.groupID}}">
            <i class="layui-icon layui-icon-edit" title="编辑"></i>
        </a>

        <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="del" href="javascript:deleteItem({{d.groupID}}, '{{ d.sourceHost.replace(/["\']/g, "")}}');">
