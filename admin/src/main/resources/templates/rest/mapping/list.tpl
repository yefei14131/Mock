...
<div class="layui-body">
    ...
    <div class="content-head">
        ...
        <div class="head-container right">
            <a href="/mock_server/rest/mapping/info.html?groupID=${groupID}&groupCode=${groupCode}">
                <svg class="icon">
...
<script>
    ...
    function cloneItem(requestID, name){
        ...
        layer.confirm(msg, function(index){
            ajaxPost('/mock_server/rest/mapping/clone.ajax', {requestID:requestID},function(response) {
                location.reload();
            })
            layer.close(index);
        })
    ...
    <script type="text/html" id="barDemo">
        <a class="layui-btn layui-btn-xs layui-bg-green" lay-event="edit"
           href="/mock_server/rest/mapping/info.html?requestID={{d.requestID}}">
            <i class="layui-icon layui-icon-edit" title="编辑"></i>
        </a>

        <a class="layui-btn layui-btn-xs" lay-event="edit" href="javascript:cloneItem({{d.requestID}}, '{{ d.path + "(" + d.memo.replace(/["\']/g, "") + ")" }}');">克隆</a>

<a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="del" href="javascript:deleteItem({{d.requestID}}, '{{ d.path + "(" + d.memo.replace(/["\']/g, "") + ")" }}');">
